...
export default function DashboardPayoutIntentsPage() {
  const snapshot = getDashboardSnapshot();
  const totalQueued = snapshot.payoutIntents.reduce((sum, intent) => sum + intent.amountCents, 0);
    ...
    <DashboardShell title="Payout Intents">
      <div className={styles.pageHeader}>
        ...
        <div className={styles.pageActions}>
          <button className={styles.primaryButton}>Approve Selected</button>
        </div>
      </div>
      ...
      <div className={styles.miniGrid} style={{ marginBottom: 18, paddingBottom: 0, borderBottom: "none" }}>
        ...
        <div className={styles.miniCard}>
          ...
          <div className={styles.statValue} style={{ fontSize: 22, color: "var(--amber)", marginBottom: 0 }}>
      ...
      <div className={styles.card}>
            ...
            <table className={styles.table}>
              ...
              <tbody>
                {snapshot.payoutIntents.map((intent) => {
                  const bucket = allocationRule?.buckets.find((entry) => entry.kind === intent.bucketKind);
                  const dotClass =
                  ...
                  return (
                    <tr key={intent.id}>
                      ...
                      <td style={{ fontFamily: "var(--font-dashboard-serif)", fontStyle: "italic", color: "var(--green)", fontSize: 15 }}>
                        {formatMoney(intent.amountCents, intent.currency)}
                      </td>
                      <td className={`${styles.mono} ${styles.muted}`}>{bucket ? Math.round(bucket.percentageBps / 100) : 0}%</td>
                      <td>
                        <span className={`${styles.tag} ${intent.status === "pending_approval" ? styles.tagAmber : styles.tagGreen}`}>
                          {intent.status}
                        </span>
                      </td>
                      <td>
                      ...
                      <td className={`${styles.faint} ${styles.mono}`} style={{ fontSize: 10 }}>
        ...
        <div className={styles.footer}>
          ...
          <span className={styles.footerValue}>{allocationRule ? "10,000 OK" : "waiting for data"}</span>
        </div>
      </div>
    </DashboardShell>
  );
